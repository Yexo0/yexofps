    ...
    <>
      ...
      <main className="relative z-[2] min-h-screen flex flex-col items-center gap-5 px-[18px] pt-[60px] pb-[100px] max-w-[1120px] mx-auto">
        ...
        <div className="w-full shop-section">
          <GlassPanel>
            <div className="mb-5">
              <h1 className="gradient-text-warm text-3xl font-extrabold mb-2">{t("title")}</h1>
            ...
            <div className="flex flex-wrap gap-3">
              <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/[0.04] border border-white/[0.06] text-xs text-white/55">
        ...
        <div className="w-full shop-section">
          <GlassPanel>
            <SectionTitle title={t("productsTitle")} />
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {PRODUCTS.map((product) => {
                const titleKey = `products.${product.id}.title` as const;
                const descKey = `products.${product.id}.desc` as const;
                return (
                  <article key={product.id} className={`product-card-premium ${product.badge ? "ring-1 ring-purple-500/20" : ""}`}>
                    <div className="relative">
                      {product.badge && (
                        <span className={`absolute top-3 left-3 z-10 px-2.5 py-1 rounded-full text-[10px] font-bold ${
                      ...
                      />
                      <span className="absolute bottom-3 right-3 px-2.5 py-1 rounded-lg bg-black/70 backdrop-blur-sm text-white text-sm font-bold border border-white/10">
                        {product.price}
                      </span>
                    </div>
                    <div className="p-4">
                      <h3 className="text-sm font-bold text-white mb-1">{t(titleKey)}</h3>
                      <p className="text-xs text-white/40 mb-3">{t(descKey)}</p>
                      <div className="flex gap-2">
                        <Link href="/liens" className="btn-premium !py-1.5 !px-3 !text-xs">
                          {t("buyContact")}
                        </Link>
                        {product.previewSrc && (
                          <button
          ...
          <GlassPanel>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <div>
                ...
                <div className="flex flex-col gap-3">
                  {REVIEWS.map((review) => (
                    <article key={review.key} className="stat-card-premium !p-4">
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <div className="flex-1">
                          ...
                          <span className="text-[10px] text-white/35 ml-2">{review.platform}</span>
                        </div>
        ...
        <div className="w-full shop-section">
          <GlassPanel>
            <SectionTitle title={t("payhipTitle")} />
        ...
        <div className="w-full shop-section">
          <GlassPanel>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
              <div className="stat-card-premium" style={{ "--stat-accent": "rgba(96, 165, 250, 0.5)" } as React.CSSProperties}>
              ...
              <div className="stat-card-premium" style={{ "--stat-accent": "rgba(244, 114, 182, 0.5)" } as React.CSSProperties}>
              ...
              <div className="stat-card-premium" style={{ "--stat-accent": "rgba(167, 139, 250, 0.5)" } as React.CSSProperties}>
...
function TrustStatCard({ stat }: { stat: TrustStat }) {
  ...
  return (
    <div className="stat-card-premium flex items-center gap-4 !p-4" style={{ "--stat-accent": colors.accent } as React.CSSProperties}>
...
function animateCount(el: HTMLSpanElement, target: number, decimals: number, duration: number) {
